...
var util = {
    ...
    getServerUrl: function getServerUrl(path) {
    ...
    renderHtml: function renderHtml(htmlTemplate, data) {
        var template = _hogan2.default.compile(htmlTemplate),
    ...
    sucTip: function sucTip(msg, time) {
        var msg = msg || '操作成功！',
            time = time || 5000;
    ...
    errTip: function errTip(msg, time) {
        var msg = msg || '操作失败！',
            time = time || 5000;
    ...
    modalPhoto: function modalPhoto(param) {
    ...
    getCookie: function getCookie(formData) {
        if (document.cookie.length > 0) {
            var arr = document.cookie.split('; ');
            for (var i = 0; i < arr.length; i++) {
                var arr2 = arr[i].split('=');
                if (arr2[0] == 'userName') {
                    formData.username = arr2[1];
    ...
    sliceArray: function sliceArray(arr, num) {
        var result = [];
    ...
    copyText: function copyText(oInp, obj) {
        oInp.value = obj.innerText;
        oInp.select();
    ...
    getList: function getList(obj) {
        ...
        for (var i = 0; i < obj.length; i++) {
            list.push({
                key: (0, _keys2.default)(obj[i])[0],
                value: (0, _values2.default)(obj[i])[0]
            });
        }
        return list;
    },

    filterList: function filterList(obj) {
        return obj;
    },

    filterSelected: function filterSelected(obj) {
        ...
        if (selectData.type || selectData.from || selectData.mark || selectData.level || obj.searchData) {
            return product.filter(function (item) {
                return (0, _keys2.default)(item).some(function (key) {
                    return (selectData.type ? item.OilType == selectData.type : true) && (selectData.from ? item.OilFrom == selectData.from : true) && (selectData.mark ? item.OilCode == selectData.mark : true) && (selectData.level ? item.OilLevel == selectData.level : true) && (obj.searchData ? String(item[key]).toLowerCase().indexOf(obj.searchData.toLowerCase()) > -1 : true);
                });
            });
        }
        return product;
    ...
    addEvent: function addEvent(obj, event, fn, type) {
        var type = type || false;
        if (obj.attachEvent) {
...
util.getFormatDate = function (param) {
    var time = param.time ? new Date(param.time) : new Date(),
...
util.handleFooter = function (obj1, obj2, pd, h) {
    var h = h || 0;
